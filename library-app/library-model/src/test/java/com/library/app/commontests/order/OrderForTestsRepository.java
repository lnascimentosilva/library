...
public final class OrderForTestsRepository {
	...
	public static Order normalizeDependencies(final Order order, final EntityManager em) {
		order.setCustomer(TestRepositoryUtils.findByPropertyNameAndValue(em, Customer.class, "name", order.getCustomer()
				.getName()));
		for (final OrderItem item : order.getItems()) {
			item.setBook(
					TestRepositoryUtils.findByPropertyNameAndValue(em, Book.class, "title", item.getBook().getTitle()));
		}
		return order;
	}
