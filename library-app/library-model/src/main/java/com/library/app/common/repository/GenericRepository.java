...
public abstract class GenericRepository<T> {
	...
	public List<Category> findAll(final String orderField) {
		return getEntityManager().createQuery("Select e From Category e Order by e." + orderField).getResultList();
	}

	public boolean alreadyExists(final String propertyName, final String propertyValue, final Long id) {
		final StringBuilder jpql = new StringBuilder();
		jpql.append(
				String.format("Select 1 From %s e where e.%s = :propertyValue", getPersistentClass().getSimpleName(),
						propertyName));
		if (id != null) {
